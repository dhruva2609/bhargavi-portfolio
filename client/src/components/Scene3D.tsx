...
const Embers = ({ mouse }: { mouse: { x: MotionValue<number>, y: MotionValue<number> } }) => {
    ...
    useFrame(() => {
        ...
        // Using performance.now() to avoid THREE.Clock deprecation warnings in newer Three.js versions
    ...
    // Bright Starry Effect
    const emberColor = '#ffffff';
...
const FloatingGlassPages = ({ scroll }: { scroll: MotionValue<number> }) => {
    ...
    useFrame(() => {
        ...
        const targetRotY = s * Math.PI * 2 + Math.sin(time * 0.2) * 0.1;
        const targetRotZ = Math.sin(s * Math.PI) * 0.4 + Math.cos(time * 0.3) * 0.05;
        ...
        groupRef.current.children.forEach((child, i) => {
            ...
            const targetChildRotX = Math.sin(time * 0.4 + i) * 0.04;
